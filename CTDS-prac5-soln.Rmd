...
The file containing the start times of video cameras forms the basis of temporal availability calculations.  Data from the camera logs are stored in a text file, `VideoStartTimes_FullDays.txt` and need to be massaged so the data can be analysed by the `activity` package (get Rowecliffe citation)

```{r datamunge, eval=solution}
library(activity)
duikers <- "VideoStartTimes_FullDays.txt"
...
daytime.boot.hr <- bootdht(model=daytime.hr0, flatfile=daytime,
                          resample_transects = TRUE, nboot=700, cores = 3,
...
- Mean and median of the bootstrap replicate distribution,
  -<p style="color: blue; font-size: 110%; font-style: italic;">Median is considerably larger than the mean of the distribution because the distribution is skewed right (long right tail in the distribution</p>
